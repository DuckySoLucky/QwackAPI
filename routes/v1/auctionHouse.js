...
const retrieveAuctions = async function (res) {
    try {
        ...
        if (error.toString().includes("ENOENT: no such file or directory, open '../data/auctions.json'")){ 
            return res.status(102).json({ status: 102, data: "Auctions haven't been updated yet. Please wait a bit" });
        } else {
...
module.exports = wrap(async function (req, res) {
    ...
    for (let query of queries) {
        ...
        if (query.startsWith('player='))  {
            if (!isUuid(query.split('=')[1])) {
                query = `player=${((await makeRequest(res, `https://api.ashcon.app/mojang/v2/user/${query.split('=')[1]}`))?.data?.uuid).replace(/-/g, '')}`;
            }
        }
    ...
    for (let auction of auctionsRes) {
        ...
        if (searchData.player) if (auction.auctioneer != searchData.player) continue;
        if (auction.bin) {
