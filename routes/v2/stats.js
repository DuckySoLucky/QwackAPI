...
module.exports = wrap(async function (req, res) {
  ...
  let uuid = req.params.uuid, profileId;
  if (!isUuid(uuid)) {
  ...
  if (profileRes.data.hasOwnProperty("profiles") && profileRes.data.profiles == null) return res.status(404).json({ status: 404, reason: `Found no SkyBlock profiles for a user with a UUID of '${uuid}' and profile of '${profileid}'`});
  ...
  if (!profileData) return res.status(404).json({ status: 404, reason: `Found no SkyBlock profiles for a user with a UUID of '${uuid}' and profile of '${profileid}'` });
  if(profileData.hasOwnProperty(uuid) && profileData[uuid].last_save != undefined) res.status(404).json({ status: 404, reason: `Found no SkyBlock profiles for a user with a UUID of '${uuid}'` });
  
  const profile = profileData.members[uuid];

  return res.status(200).json({ status: 200, data: await getStats(player, profileData, profile, uuid, res) });
});
