...
module.exports = (req, res, next) => {
  ...
  if (
    ...
    AuthTokens.includes(req.headers.authorization)
  ) {
  ...
  if (
    ...
    AuthTokens.includes(req.query.key?.toString())
  ) {
  ...
  return res.status(400).json({
    status: 400,
    reason:
      'Missing "key" query parameter, or an "authorization" header with a valid SkyHelper API key',
  });
};
